...
public class App {
    ...
    public static void buscarJogoPorNome(ListaObj<Jogo> jogos){
        System.out.println("Digite o que quer buscar:");
        String busca = leitor.next();
        ListaObj<Jogo> achados = new ListaObj<>(jogos.getTamanho());
        for (int i = 0; i < jogos.getTamanho(); i++) {
            if (jogos.getElemento(i).getNome().toLowerCase().equals(busca.toLowerCase()) ||
            jogos.getElemento(i).getNome().toLowerCase().contains(busca.toLowerCase())) {
                achados.adiciona(jogos.getElemento(i));
            }
        }
        exibeJogoNaLista(achados);
    }
